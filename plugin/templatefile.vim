...
function! LoadTemplateFile()
	...
	let datetime   = Escape(strftime("%c"))
	let year       = Escape(strftime("%Y"))
	let time       = Escape(strftime("%T"))
	let modtime    = Escape(strftime("%c", getftime(expand("%:p"))))
	...
	silent! execute "%s/@DATE@/"          . datetime   . "/g"
	silent! execute "%s/@DATETIME@/"      . datetime   . "/g"
	silent! execute "%s/@YEAR@/"          . year       . "/g"
	silent! execute "%s/@TIME@/"          . time       . "/g"
	silent! execute "%s/@MODTIME@/"       . modtime    . "/g"
