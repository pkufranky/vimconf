...
" added by franky to simulate ultraedit {{{
if !exists('g:miniBufExplDoubleClickDelete')
  let g:miniBufExplDoubleClickDelete = 1
endif 
if g:miniBufExplDoubleClickDelete == 1
    let s:clickmap = '<ESC>:if bufname("%") == "-MiniBufExplorer-" <bar> call <SID>MBEDeleteBuffer() <bar> endif <CR>'
    if maparg('<2-LEFTMOUSE>', 'n') == '' 
        " no mapping for leftmouse
        "exec ':nnoremap <silent> <LEFTMOUSE> <LEFTMOUSE>' . s:clickmap
        exec ':nnoremap <silent> <2-LEFTMOUSE> <2-LEFTMOUSE>' . s:clickmap
        "nnoremap <buffer> <2-LEFTMOUSE> :call <SID>MBEDeleteBuffer()<CR>:<BS>
    endif
endif
" }}}
